...
if ( is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) ) {
    ...
    if (!defined('cf7BOT_VIEWS_DIR'))
        define('cf7BOT_VIEWS_DIR', cf7bot_root_dir('views/'));

    if (!defined('cf7BOT_VERSION'))
        define('cf7BOT_VERSION', '1.2');
	...
	function cf7bot_admin_panel ( $panels ) {
		$new_page = array(
			'bot-forms-addon' => array(
				'title' => __( 'Conversational Form', 'contact-form-7' ),
	...
	function cf7bot_admin_panel_content( $cf7 ) {
        ...
        $form_bot_icon = get_post_meta($post_id, "_cf7bot_bot_icon", true);
        $form_user_icon = get_post_meta($post_id, "_cf7bot_user_icon", true);
		...
		$search_replace = array(
            ...
            '{bot_icon}' => $form_bot_icon,
            '{bot_icon_image}' => (wp_get_attachment_image_url($form_user_icon) != '') ? '<img id="bot_icon_img" src="'.wp_get_attachment_image_url($form_bot_icon).'" alt="">' : '',
            '{user_icon}' => $form_user_icon,
            '{user_icon_image}' => (wp_get_attachment_image_url($form_user_icon) != '') ? '<img id="user_icon_img" src="'.wp_get_attachment_image_url($form_user_icon).'" alt="">' : '',
	...
	function cf7bot_admin_save_form( $cf7 ) {
        ...
        update_post_meta($post_id, '_cf7bot_bot_icon', $_POST['cf7bot_bot_icon']);
        update_post_meta($post_id, '_cf7bot_user_icon', $_POST['cf7bot_user_icon']);
	...
	function wpcf7_form_response_output($output, $class, $content, $WPCF7_ContactForm){
        ...
        $form_bot_icon = get_post_meta($post_id, "_cf7bot_bot_icon", true);
        $form_user_icon = get_post_meta($post_id, "_cf7bot_user_icon", true);
        $userImage = (wp_get_attachment_image_url($form_user_icon) != '') ? wp_get_attachment_image_url($form_user_icon) : cf7bot_root_url('assets/images/human.png');
        $robotImage = (wp_get_attachment_image_url($form_bot_icon) != '') ? wp_get_attachment_image_url($form_bot_icon) : cf7bot_root_url('assets/images/robot.png');
                    ...
                    setTimeout(function() {
                        ...
                        $(document).on("click", "<?php echo $toggle; ?>", function(){
                            if(!window.ConversationalForm){
                                    ...
                                    <?php if($form_outer == ''): ?>
                                    context: document.getElementById('<?php echo $form_id; ?>').parentElement,
                                    <?php else: ?>
                                    context: document.getElementById(<?php echo $form_outer ?>),
                                    <?php endif; ?>
                                    userImage: "<?php echo $userImage; ?>",
                                    robotImage: "<?php echo $robotImage; ?>"
                                });
                            }
    ...
    function sample_admin_notice__success($checkonly = null) {
        $plugins = array(
        ...
        $format = '<div class="notice notice-error"><p><i>Conversational Form Add-on For Contact Form 7</i> required plugin %s: %s</p></div>';
        $out = '';
